...
import (
	...
	"encoding/hex"
	"encoding/json"
	bc "first-simple-blockchain/blockchain"
	nt "first-simple-blockchain/network"
	"fmt"
...
func handleServer(conn nt.Conn, pack *nt.Package) {
	...
	nt.Handle(ADD_BLOCK, conn, pack, getBlock)
...
func addBlock(pack *nt.Package) string {
	...
	if !block.IsValid(Chain, Chain.Size()) {
		...
		if currSize < uint64(num) {
			go compareChains(splited[0], uint64(num)) // splited[0] address of sender
			return "ok"
		}
		return "fail"
	}
...
func compareChains(address string, num uint64) {
	...
	chain.AddBlock(genesis) // index == 1
	...
	Mutex.Lock()     // refresh node
	Chain.DB.Close() // close BlockChain
	os.Remove(Filename)
...
func writeFile(filename, data string) error {
	return ioutil.WriteFile(filename, []byte(data), 0644) // 0644 -- access rights
}
